...
class MentionProposalModule(nn.Module):
    ...
    def get_mention_embeddings(self, encoded_doc: Tensor, ment_starts: Tensor, ment_ends: Tensor
        ...
        if len(ment_starts) != len(ment_ends):
            raise ValueError("Lengths of ment_starts and ment_ends must be the same.")
    
        # Initialize an empty list to store the mention embeddings for each candidate span.
        mention_embeddings_list = []
        
        # Iterate over each candidate span.
        for start, end in zip(ment_starts, ment_ends):
            # Extract the embeddings between start and end (inclusive) from encoded_doc.
            mention_embeddings = encoded_doc[start : end + 1]  # +1 to include the end token.
            mention_embeddings_list.append(mention_embeddings)
        
        return mention_embeddings_list
    ...
    def pred_mentions(
        ...
        if self.training or eval_loss:
            ...
            if not mention_params.use_topk:
                ...
                true_positives = ((mentions_proposed == 1) & (filt_gold_mentions == 1)).sum().item()
